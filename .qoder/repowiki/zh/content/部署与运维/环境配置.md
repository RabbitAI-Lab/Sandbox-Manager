...
## 更新摘要
**变更内容**
- 新增环境变量 OPENSANDBOX_PROTOCOL、OPENSANDBOX_USE_SERVER_PROXY、OPENSANDBOX_REQUEST_TIMEOUT_SECONDS 的配置支持
- 更新配置加载器以支持新的 OpenSandbox 相关环境变量
- 增强设置界面以支持协议选择和超时配置
- 完善 Helm 配置模板以包含新的环境变量
...
- 前端设置界面：apps/frontend/src/components/settings/ServerSettingsModal.tsx
...
subgraph "前端界面"
UI["设置界面<br/>ServerSettingsModal.tsx"]
end
CFG --> SRV
SRV --> SETUP
SETUP --> SSET
...
- [infra/helm/sandbox-platform/values.yaml:1-48](file://infra/helm/sandbox-platform/values.yaml#L1-L48)
- [infra/helm/sandbox-platform/templates/backend-configmap.yaml:1-14](file://infra/helm/sandbox-platform/templates/backend-configmap.yaml#L1-L14)
...
- [apps/frontend/src/components/settings/ServerSettingsModal.tsx:270-464](file://apps/frontend/src/components/settings/ServerSettingsModal.tsx#L270-L464)
...
- [apps/backend/src/routes/setup.ts:17-153](file://apps/backend/src/routes/setup.ts#L17-L153)
- [apps/backend/src/services/setupService.ts:27-154](file://apps/backend/src/services/setupService.ts#L27-L154)
...
- [apps/backend/src/services/setupService.ts:108-154](file://apps/backend/src/services/setupService.ts#L108-L154)
- [infra/helm/sandbox-platform/templates/backend-configmap.yaml:7-14](file://infra/helm/sandbox-platform/templates/backend-configmap.yaml#L7-L14)
...
**章节来源**
- [apps/backend/src/routes/setup.ts:17-153](file://apps/backend/src/routes/setup.ts#L17-L153)
- [apps/backend/src/services/setupService.ts:68-154](file://apps/backend/src/services/setupService.ts#L68-L154)
...
- [infra/helm/sandbox-platform/values-production.yaml:21-27](file://infra/helm/sandbox-platform/values-production.yaml#L21-L27)
...
## 结论
本项目通过统一的配置加载器与明确的优先级策略，将 .env、Kubernetes ConfigMap/Secret、Docker 环境变量与进程环境变量整合为一致的运行时配置来源。配合设置流程与热更新机制，实现了从开发到生产的无缝配置管理。新增的 OPENSANDBOX_PROTOCOL、OPENSANDBOX_USE_SERVER_PROXY 和 OPENSANDBOX_REQUEST_TIMEOUT_SECONDS 环境变量进一步增强了对 OpenSandbox 服务的灵活配置能力，支持不同的部署场景和网络架构需求。建议在生产中严格分离敏感与非敏感配置，启用 HTTPS 与最小权限原则，并结合自动扩缩容与健康检查保障稳定性。
...
- [scripts/dev-backend.sh:14-16](file://scripts/dev-backend.sh#L14-L16)
- [apps/backend/Dockerfile:23-24](file://apps/backend/Dockerfile#L23-L24)
- [infra/helm/sandbox-platform/values-production.yaml:15-27](file://infra/helm/sandbox-platform/values-production.yaml#L15-L27)
- [infra/helm/sandbox-platform/values.yaml:27-34](file://infra/helm/sandbox-platform/values.yaml#L27-L34)
...
- [apps/backend/src/config.ts:60-61](file://apps/backend/src/config.ts#L60-L61)
- [apps/backend/src/server.ts:40](file://apps/backend/src/server.ts#L40)
- [apps/backend/src/services/setupService.ts:131-154](file://apps/backend/src/services/setupService.ts#L131-L154)
...
- Kubernetes ConfigMap（生产）
  ...
  - OPENSANDBOX_REQUEST_TIMEOUT_SECONDS: "600"
  - PORT: "3000"
...
- OPENSANDBOX_PROTOCOL
  - 类型：字符串枚举（http | https）
  - 默认值：http
  - 用途：指定 OpenSandbox 服务通信协议，支持 HTTP 和 HTTPS
  - 影响范围：所有与 OpenSandbox 服务的通信
- OPENSANDBOX_USE_SERVER_PROXY
  - 类型：布尔值
  - 默认值：true
  - 用途：控制是否使用服务端代理转发 OpenSandbox 请求
  - 影响范围：网络请求路由策略
- OPENSANDBOX_REQUEST_TIMEOUT_SECONDS
  - 类型：整数值（秒）
  - 默认值：300
  - 用途：设置 OpenSandbox 请求超时时间
  - 影响范围：API 调用超时控制

**章节来源**
- [apps/backend/src/config.ts:58-61](file://apps/backend/src/config.ts#L58-L61)
- [apps/backend/src/services/setupService.ts:108-154](file://apps/backend/src/services/setupService.ts#L108-L154)
- [apps/frontend/src/components/settings/ServerSettingsModal.tsx:277-296](file://apps/frontend/src/components/settings/ServerSettingsModal.tsx#L277-L296)
- [infra/helm/sandbox-platform/values.yaml:42-44](file://infra/helm/sandbox-platform/values.yaml#L42-L44)